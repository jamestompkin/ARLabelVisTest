...
def _render_and_crop(cfg, out_name: str, title: str):
    tmp = FIG_DIR / "_tmp_shortpaper_zoom"
    tmp.mkdir(exist_ok=True)
    full = tmp / f"{out_name}.full.png"
    render_srgb_cube_isometric(
        lut_to_srgb_u8(get_lut(cfg), cfg), save_path=full, title=title,
    )
    ...
    crop.save(shortpaper_path("smoothing", out_name))
    print(f"wrote smoothing/{out_name}")


def main():
    sigma_4 = next(c for c in LUT_CIELAB_GAUSSIAN_SIGMAS if c.sigma == 4.0)
