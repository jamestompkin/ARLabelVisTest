...
def main():
    alphas = [c.metric_rgd_alpha_hat for c in LUT_ALPHA_SWEEP]
    grad_max, grad_avg, sat, val = [], [], [], []
    for cfg in LUT_ALPHA_SWEEP:
        gm, ga, s, v = _metrics(lut_to_srgb_u8(get_lut(cfg), cfg.output_space))
    ...
    plt.tight_layout()
    out = fig_path("alpha_plots", "alpha_plots.png")
