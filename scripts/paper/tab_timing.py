...
from arlabelvis.luts import DEFAULT_CACHE, get_lut, lut_to_srgb_u8
from scripts.paper._paths import tab_path


NAIVE_SIGMA = 3.0   # matches tab_scene


def _invalidate(cfg) -> None:
    """Delete a LUT's cache files so the next get_lut call rebuilds."""
    key = cfg.key()
    for p in DEFAULT_CACHE.root.glob(f"{key}*"):
        p.unlink()
...
def _time_gaussian_filter(base_cfg, sigma: float) -> float:
    """Wall-clock of a 3D Gaussian filter over the ``base_cfg`` LUT. Assumes
    the LUT is already cached — the caller should have timed the base build
    separately and we reuse it here to avoid a redundant ~1h rebuild at
    interval=1."""
    lut_u8 = lut_to_srgb_u8(get_lut(base_cfg), base_cfg.output_space)
    t0 = time.perf_counter()
...
def main():
    ...
    print("[tab_timing] timing Smoothed baseline Gaussian pass "
          "(reuses cached Original)...")
    gauss_s = _time_gaussian_filter(LUT_CIELAB_DELTAE00, NAIVE_SIGMA)
    smoothed_s = original_s + gauss_s
    print(f"    {gauss_s:.1f} s (Gaussian) + {original_s:.1f} s "
          f"(Original rebuild) = {smoothed_s:.1f} s")
    ...
    rows = [
        ...
        (r"Smoothed (baseline)",       smoothed_s,
         fr"Original construction + 3D Gaussian ($\sigma={NAIVE_SIGMA:g}$)"),
        (r"Ours",                      ours_s,
    ...
    out = tab_path("timing", "timing.tex")
    out.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"wrote {out}")
