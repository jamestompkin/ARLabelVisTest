...
def process_scene_video(
    ...
    * ``cec_source="background"``: CEC is computed from pixels under a
    ...
    if cec_source == "background" and background_mask_path is None:
        import warnings
        warnings.warn(
            "cec_source='background' with no background_mask_path: falling back "
            "to NOT-label, which samples ~everything that isn't the label. For a "
            "faithful Kwon comparison use cec_source='label' (pixels under the "
            "label mask), or supply the actual background_mask_path that Kwon's "
            "granularity=2 pipeline used.",
            stacklevel=2,
        )
